...
// tag::add[]
func (fs *neo4jFavoriteService) Save(userId, movieId string) (movie Movie, err error) {
...
// tag::all[]
func (fs *neo4jFavoriteService) FindAllByUserId(userId string, page *paging.Paging) (movies []Movie, err error) {
	session := fs.driver.NewSession(neo4j.SessionConfig{})
	defer func() {
		err = ioutils.DeferredClose(session, err)
	}()
